...
bool ValidateOutputPath(const std::string& path, std::string& error) {
  ...
  if (std::filesystem::exists(out, ec) && std::filesystem::is_directory(out, ec)) {
    error = "Output path points to a directory: " + path;
    return false;
  }
  const std::filesystem::path parent = out.has_parent_path() ? out.parent_path() : ".";
...
bool ValidatePreflopRanges(std::string_view project_root, std::string& error) {
  using poker_solver::solver::Position;

  if (project_root.empty()) {
    error = "PROJECT_SOURCE_DIR was not set at build time.";
    return false;
  }

  std::vector<std::string> required_paths;
  required_paths.reserve(32);
  ...
  for (const auto opener : kOpeners) {
    required_paths.push_back(poker_solver::solver::detail::OpenRangePath(project_root, opener));
    for (const auto defender : poker_solver::solver::detail::AvailableDefenders(opener)) {
      required_paths.push_back(
          poker_solver::solver::detail::VsOpenRangePath(project_root, defender, opener));
    }
  }
  ...
  for (const auto& path : required_paths) {
    try {
      (void)poker_solver::io::LoadPreflopRange(path);
  ...
  if (!failures.empty()) {
    error = "Preflop ranges are missing or invalid.\n";
    error += "Expected these files under: " + std::string(project_root) + "/preflop-ranges/\n";
    error += "Failures (" + std::to_string(failures.size()) + "):\n";
    const std::size_t limit = 8;
    for (std::size_t i = 0; i < failures.size() && i < limit; ++i) {
      error += "  - " + failures[i] + "\n";
    }
    if (failures.size() > limit) {
      error += "  - ... (" + std::to_string(failures.size() - limit) + " more)\n";
    }
    return false;
  }
...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    if (!ValidateOutputPath(options.output, path_error)) {
      LOG_ERROR(path_error);
      return EXIT_FAILURE;
    }

    std::string ranges_error;
    if (!ValidatePreflopRanges(PROJECT_SOURCE_DIR, ranges_error)) {
      LOG_ERROR(ranges_error);
      return EXIT_FAILURE;
    }
    ...
    while (written < target) {
      ...
      poker_solver::solver::Scenario scenario;
      try {
        scenario = poker_solver::solver::GenerateScenario(rng, scenario_config);
      } catch (const std::exception& e) {
        ++failed_scenarios;
        if (failed_scenarios <= 5) {
          LOG_WARN(std::string("Scenario generation failed: ") + e.what());
        }
        continue;
      }
      ...
      const auto root_strategy =
          poker_solver::solver::SolveRootStrategy(scenario.root_state, root_opt);
      ...
      out.hero_cards =
          poker_solver::solver::CanonicalTwoCards(scenario.hero_cards[0], scenario.hero_cards[1]);
      out.villain_cards =
      ...
      if (options.progress_every > 0 &&
          (written % options.progress_every == 0 || written == target)) {
        ...
        LOG_INFO("Progress: wrote " + std::to_string(written) + "/" + std::to_string(target) +
                 ...
                 ", attempts=" + std::to_string(attempts) + ", failed_scenarios=" +
                 std::to_string(failed_scenarios) + ", elapsed_s=" + std::to_string(elapsed_s) +
                 ")");
      }
    }
